    ...
    ILogger<FileCodexThreadStore> logger) : ICodexThreadStore
{
    ...
    public async Task<CodexThreadRecord?> TryGetByKeyAsync(string threadKey, string? threadStorePath, CancellationToken cancellationToken)
    {
        ...
        {
            throw new RuntimeInvalidRequestException("ThreadKey must be a non-empty string.", RuntimeName);
        }
    ...
    public async Task SaveAsync(CodexThreadRecord record, string? threadStorePath, CancellationToken cancellationToken)
    {
        ...
        {
            throw new RuntimeInvalidRequestException("ThreadId must be a non-empty string.", RuntimeName);
        }
    ...
    private string ResolveStorePath(string? threadStorePath)
    {
        ...
        {
            throw new RuntimeOperationException("Failed to resolve LocalApplicationData for codex thread store.", RuntimeName);
        }

        return Path.Combine(localAppData, "MultiCodingAgentFacade", "CodexAppServer", "threads.json");
    }
    ...
    private async Task<ThreadStoreDocument> ReadDocumentAsync(string path, CancellationToken cancellationToken)
    {
        ...
        catch (JsonException ex)
        {
            ...
            throw new RuntimeOperationException($"Failed to parse codex thread store '{path}'.", RuntimeName, null, null, null, ex);
        }
        catch (IOException ex)
        {
            logger.LogError("Failed to read codex thread store. Exception={Exception}", ex.ToString());
            throw new RuntimeOperationException($"Failed to read codex thread store '{path}'.", RuntimeName, null, null, null, ex);
        }
        catch (UnauthorizedAccessException ex)
        {
            logger.LogError("Failed to access codex thread store. Exception={Exception}", ex.ToString());
            throw new RuntimeOperationException($"Access denied for codex thread store '{path}'.", RuntimeName, null, null, null, ex);
        }
        catch (System.NotSupportedException ex)
        {
            logger.LogError("Invalid codex thread store path. Exception={Exception}", ex.ToString());
            throw new RuntimeOperationException($"Invalid codex thread store path '{path}'.", RuntimeName, null, null, null, ex);
        }
    }
    ...
    private async Task WriteDocumentAsync(string path, ThreadStoreDocument document, CancellationToken cancellationToken)
    {
        ...
        catch (IOException ex)
        {
            ...
            throw new RuntimeOperationException($"Failed to write codex thread store '{path}'.", RuntimeName, null, null, null, ex);
        }
        catch (UnauthorizedAccessException ex)
        {

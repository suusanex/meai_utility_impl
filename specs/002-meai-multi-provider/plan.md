...
**Performance Goals**: チャット応答のオーバーヘッド < 10ms（プロバイダー呼び出しを除く）、メモリオーバーヘッド < 5MB  
**Constraints**: 
...
- 📋 **検証方針**:
  ...
  - User Story 4（Embedding）: P3（将来拡張）のため Phase 2 以降で検証計画を策定
...
| **IProviderFactory** | プロバイダー名から具体的なIChatClient実装を生成 | 公開 |
...
**方針**: MEAI標準のロール名をそのまま各プロバイダーへ渡す（現時点で差異なし）
...
|-----------|--------|-------------|---------|
| 認証失敗 | `AuthenticationException` | 401/403 | プロバイダー名、エラーメッセージ |
| レート制限 | `RateLimitException` | 429 | Retry-After ヘッダー、プロバイダー名 |
| 無効リクエスト | `InvalidRequestException` | 400 | 検証エラー詳細、プロバイダー名 |
| サーバーエラー | `ProviderException` | 500-599 | ステータスコード、レスポンス本文 |
| タイムアウト | `TimeoutException` | N/A | タイムアウト時間、プロバイダー名 |
| 未対応機能 | `NotSupportedException` | N/A | 機能名、プロバイダー名 |
| Copilot起動失敗 | `CopilotRuntimeException` | N/A | CLIパス、プロセス終了コード |

**例外設計方針**:
- 全例外は `MultiProviderException` 基底クラスから派生
- `ProviderName`、`TraceId`、`Timestamp` プロパティを必須で含む
...
- アダプタは `CommonChatOptions` → OpenAI固有オプションへの変換のみを担当
...
- OpenAI → Azure OpenAI → Copilot の3パターンを同じコードで実行
...
| C_Factory | Component | Provider Factory | 選択済みプロバイダー実装を生成 | src/MeAiUtility.MultiProvider/Abstractions | DI container | Factory |
...
| S-030 | Azure拡張パラメータを適用する | Provider=AzureOpenAI | 拡張付き実行 | data_sources等が反映 | Application, P_AzureOpenAI, X_ModelAPI, T_Logger | common + extension parameters | 型不一致は警告、不正要求は例外 | extension_applied / extension_validation_warning | spec FR-006 |
...
| C_Factory | src/MeAiUtility.MultiProvider/Abstractions | IChatClient生成 | 未知Provider拒否 |
...
| Check ID | 判定 | 根拠 |
|---|---|---|
| A1: Plan内の箱はVocabularyで定義済みか | Yes | Vocabulary表 + S-010〜S-070 |
...
| C2: Scenario→Vocabulary→住所で追跡可能か | Yes | S-010〜S-070 + Mapping表 |

### 7. TBD / Open Questions

- GitHub Copilot runtimeの配布形態差異（開発環境差）をどこまでテスト対象化するか
- OpenAI互換エンドポイントの「互換性保証範囲」をドキュメント上でどの粒度まで明示するか
- Embedding（P3）を追加する際のVocabulary拡張（既存ID流用か新規Container追加か）

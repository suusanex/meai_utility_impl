...
Codex App Server で stdout 受信行の JSON-RPC パースに失敗すると、`RuntimeOperationException` が上位へ伝播します。`RuntimeOperationException.ResponseBody` には `LineLength`、`LinePrefix`、`LineSuffix`、`ParseError`、`RequestId`、`TraceId`、`Command`、`Arguments`、`ExitCode`、`StderrTail` が含まれます。
